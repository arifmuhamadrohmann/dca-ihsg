...
export default function SummaryCard({ result }: SummaryCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`grid grid-cols-2 gap-3 mt-4 pt-4 border-t ${isPositive ? 'border-[#9fe870]/40' : 'border-white/10'}`}
      >
        {[
          ...
          <div key={label}>
            <p
              className={`text-[11px] font-semibold ${isPositive ? 'text-[#054d28]' : 'text-[#868685]'}`}
            >
              {label}
            </p>
            <p
              className={`text-[14px] font-semibold mt-0.5 ${isPositive ? 'text-[#0e0f0c]' : 'text-[#e8ebe6]'}`}
            >
              {value}
            </p>
          </div>
